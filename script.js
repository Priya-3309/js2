...
function update(res) {
  ...
  for (i = 0; i < cardlist.length; i++) {
    let cb = `<div style="width:18rem;'border:none'" class="card m-3 shadow">
    <div style='background-color:rgb(59, 218, 218)'  class="card-header text-center text-light">${cardlist[i][1].name}</div>
    <div class="card-body text-center bg-primary-subtle text-bg-info">${cardlist[i][1].body}</div>
    <div style='background-color:rgb(59, 218, 218)' class="card-footer text-center text-light">${cardlist[i][1].email}</div>
    </div></div>`;
    card.innerHTML += cb;
  }
}
